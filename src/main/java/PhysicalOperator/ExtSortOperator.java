...
public class ExtSortOperator extends SortOperator {
  ...
  private File firstRun(PhysicalOperator child) {
    ...
    try {
      while (flag) {
        ...
        Collections.sort(
            lst, new TupleComparator(orderByElements, this.getOutputSchema(), this.aliasMap));
        for (Tuple e : lst) {
          w.writeTuple(e);
        }
        ptr++;
  ...
  private File kRun(File lastRun, int B, int depth) {
    ...
    if (files.length == 1) return lastRun;
    File nextRun = createDir(lastRun, depth + "-run");
    List<Tuple> res = new LinkedList<>();
    PriorityQueue<Node> pq =
        new PriorityQueue<>(new NodeComparator(orderByElements, getOutputSchema(), aliasMap));
    TupleReader r = null;
    TupleWriter w = null;
    ...
    try {
      ...
      if (lst != null) {
        for (TupleReader rd : lst) if (rd != null) rd.close();
      }
    }
  ...
  private class NodeComparator implements Comparator<Node> {
    ...
    public NodeComparator(
        List<OrderByElement> orderByElements, ArrayList<Column> cols, Map<String, Table> aliasMap) {
