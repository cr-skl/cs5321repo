...
// * works for Statements that are Selects, and specifically PlainSelects. Could implement the
// visitor
// * pattern on the statement, but doesn't for simplicity as we do not handle nesting or other
// complex
// * query features.
// *
// * <p>Query plan fixes join order to the order found in the from clause and uses a left deep tree
// * join. Maximally pushes selections on individual relations and evaluates join conditions as
// early
// * as possible in the join tree. Projections (if any) are not pushed and evaluated in a single
// * projection operator after the last join. Finally, sorting and duplicate elimination are added
// if
// * needed.
// *
// * <p>For the subset of SQL which is supported as well as assumptions on semantics, see the
// Project
// * 2 student instructions, Section 2.1
// */
// public class QueryPlanBuilder_old {
//
//  public QueryPlanBuilder_old() {}
//
//  /**
...
//        if (combinedCondition != null) {
//          treeBuilder.visit(new TNLJ_Operator(subTreeBuilder.getRoot(), combinedCondition));
//        } else {
//          // If no specific condition, just join without conditions
//          treeBuilder.visit(new TNLJ_Operator(subTreeBuilder.getRoot(), null));
//        }
...
//
//  /**
//   * Parse the whole expression in the WHERE into List<Expression> that are delimited by AND
// Return
