...
public class ConfigHelper {
  ...
  public String inputDir, outputDir, tempDir;
  public URI outputURI;
  public boolean buildIndex, execQuery;
  public Path sqlPath, dbPath, schemaPath, index_info_Path, index_Path;

  public ConfigHelper(String inputDir) throws URISyntaxException, IOException {
    readConfig(inputDir);
  }
  public void readConfig(String inputDir) {
    ...
    try {
      ...
      BufferedReader br = new BufferedReader(new FileReader(configDir));
      // read JOIN params, in the first line, 0/1/2 -> TNLJ/BNLJ/SMJ
      ...
      // read SORT params, in the second line, 0/1 -> internal/external sort
...
//  }
  ...
  private void buildIndexOne(String table, String key, boolean clustered, int d) {
    ...
    TupleReaderBinImpl tupleReader =
        new TupleReaderBinImpl(DBCatalog.getInstance().getFileForTable(table));
    ...
    if (!clustered) {
      ...
      for (int j = 0; j < whole.size(); ) {
        ...
        for (;
            k < whole.size()
                && mt.getElementAtIndex(keyIdx) == (next = whole.get(k)).getElementAtIndex(keyIdx);
            k++) {
      ...
      IndexWriter indexWriter =
          new IndexWriterBinImpl(new File(index_Path.toString(), fileName), d);
  ...
  private Queue<Node> buildIndexPerLayer(
      IndexWriter indexWriter, Queue<Node> childNodeQueue, int d, int[] childPtr, int[] pageCnt) {
